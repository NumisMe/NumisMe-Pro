...
contract Controller is IController {
    ...
    // vault => Vault
    mapping(address => VaultDetail) public _vaultDetails;
    // strategy => vault
    mapping(address => address) public _vaultStrategies;
    // asset1 => asset2 converter
    mapping(address => mapping(address => address)) public _converter;
        ...
        onlyStrategist
    {
        ...
        uint256 tail = vaultDetail.strategies.length - 1;
        ...
        onlyStrategy(_strategy)
    {
        ...
        if (_balance > _cap && _cap != 0) {
            uint256 _diff = _balance - _cap;
            IStrategy(_strategy).withdraw(_diff);
            updateBalance(_vault, _strategy);
            _balance = IStrategy(_strategy).balanceOf();
            _vaultDetails[_vault].balance = _vaultDetails[_vault].balance - _diff;
            address _want = IStrategy(_strategy).want();
            if (_token == address(0)) _token = _want;
            _balance = IERC20(_want).balanceOf(address(this));
            if (_want != _token) {
                IConverter converter = IConverter(_converter[_want][_token]);
                require(address(converter) != address(0), "No converter");
                IERC20(_want).safeTransfer(address(converter), _balance);
                _balance = converter.convert();
     ...
     * @param _from asset being converted from
     * @param _to asset being converted to
     * @param _converterStrategy The address of the converter
     */
    function setConverter(
        address _from,
        address _to,
        address _converterStrategy
    )
        external
        notHalted
        onlyStrategist
    {
        _converter[_from][_to] = _converterStrategy;
    }
     ...
     * @param _strategy The address of the strategy
     * @param _token The token address to convert to
     */
        ...
        onlyStrategy(_strategy)
    {
        ...
        if (_token != _want) {
            IConverter converter = IConverter(_converter[_want][_token]);
            require(address(converter) != address(0), "No converter");
            IERC20(_want).safeTransfer(address(converter), _amount);
            _amount = converter.convert();
        ...
        if (_balance >= _amount) {
            _vaultDetails[_vault].balance = _balance - _amount;
        } else {
        ...
        onlyStrategy(_strategy)
    {
        ...
        address _vault = _vaultStrategies[_strategy];
        _vaultDetails[_vault].balance = _vaultDetails[_vault].balance + _after - _before;
        ...
        returns (uint256)
    {
        ...
        address _want = IStrategy(_strategy).want();
        if (_want != _token) {
            IConverter converter = IConverter(_converter[_token][_want]);
            require(address(converter) != address(0), "No converter");
        ...
        _vaultDetails[msg.sender].balance = _vaultDetails[msg.sender].balance + _amount;
