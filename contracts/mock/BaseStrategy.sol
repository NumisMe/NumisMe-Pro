...
abstract contract BaseStrategy is IStrategy {
    ...
    )
    {
        ...
        for(uint i = 0; i < _routerArray.length; i++) {
            IERC20(_weth).approve(address(_routerArray[i]), type(uint256).max);
        }
    }
        ...
        external
    {
        ...
        for(uint i = 0; i < _routerLength; i++) {
            ...
            for(uint j = 0; j < _tokenArrayLength; j++) {
                IERC20(_tokenArray[j]).approve(_router, type(uint256).max);
            }
        }
    ...
    function skim()
        ...
        onlyController
    {
        IERC20(want).transfer(controller, balanceOfWant());
    }
        ...
        onlyController
    {
        ...
        _assetToken.transfer(controller, _balance);
    }
        ...
        onlyController
    {
        ...
        IERC20(want).transfer(controller, _balance);
    }
        ...
        returns (uint256 _wethBal)
    {
        ...
        if (_wethBal > 0) {
            ...
            if (treasuryFee > 0 && treasury != address(0)) {
                _fee = _wethBal * treasuryFee / ONE_HUNDRED_PERCENT;

                IERC20(weth).transfer(treasury, _fee);
            }
